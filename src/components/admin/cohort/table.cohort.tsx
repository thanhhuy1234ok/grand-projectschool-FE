...
import { deleteUserAPI, getCohortsAPI, getUsersAPI } from "@/services/api";
...
import ModalCohort from "./model.cohort";


const TableCohort = () => {
    const [openModal, setOpenModal] = useState<boolean>(false);
    const [openModalImport, setOpenModalImport] = useState(false);
    const [dataUpdate, setDataUpdate] = useState<ICohort | null>(null);
    ...
    const [currentDataTable, setCurrentDataTable] = useState<ICohort[]>([]);
    ...
    const columns: ProColumns<ICohort>[] = [
        {
            title: 'Id',
            dataIndex: 'id',
            key: 'id',
    ...
    return (
        <>
            <DataTable<ICohort>
                actionRef={tableRef}
                headerTitle="Danh sách Năm nhập học"
                rowKey="id"
                columns={columns}
                dataSource={currentDataTable}
                request={async (params, sort, filter): Promise<any> => {
                    const query = buildQuery(params, sort, filter);
                    const res = await getCohortsAPI(query);
                    if (res.data) {
